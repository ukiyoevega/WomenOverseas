...
struct TabbarView: View {
    ...
    var body: some View {
        TabView(selection: $selectedTab) {
            Webview(url: url("http://womenoverseas.com"), sedKey: nil).tabItem {
                self.tabbarItem(text: "Home", image: "house.fill")
            }.tag(Tab.latest)
            Webview(url: url("http://womenoverseas.com/latest"), sedKey: nil).tabItem {
                self.tabbarItem(text: "Latest", image: "square.stack.fill")
            }.tag(Tab.latest)
            Webview(url: url("http://womenoverseas.com/tag/精华贴"), sedKey: nil).tabItem {
                self.tabbarItem(text: "Featured", image: "star.circle")
            }.tag(Tab.discover)
            Webview(url: url("https://womenoverseas.com/discourse-post-event/events"), sedKey: nil).tabItem {
